...
import { emailSchema, forgotPasswordSchema, loginSchema, registerSchema, resetPasswordSchema } from "../lib/schemas/authSchemas";
...
authRouter.post('/reset-password', async (req, res) => {
    ...
    try {
        ...
        const foundToken = await db.query.verificationCode.findFirst({ where: eq(verificationCode.token, token) })
...
authRouter.post("/check-email", async (req, res) => {
    try {
        ...
        const foundUser = await db.query.user.findFirst({ where: eq(user.email, body.email) });
